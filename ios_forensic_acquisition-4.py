...
class iOSDeviceInterface:
    ...
    def get_installed_apps(self, app_type="user"):
        ...
        Supporta sia la sintassi nuova (list --user) che quella legacy (-l -o list_user).
        """
        if not self.available_tools.get("ideviceinstaller"):
            return False, "ideviceinstaller non disponibile"
        
        # Rileva versione: la nuova usa 'list' come subcommand
        # Prova prima la sintassi nuova, se fallisce usa la legacy
        cmd_new = ["ideviceinstaller"]
        if self.udid:
            cmd_new.extend(["-u", self.udid])
        cmd_new.append("list")
        
        if app_type == "all":
            cmd_new.append("--all")
        elif app_type == "system":
            cmd_new.append("--system")
        else:
            cmd_new.append("--user")
        
        stdout, stderr, rc = run_command(cmd_new, timeout=30)
        
        # Fallback alla sintassi legacy se la nuova fallisce
        if rc != 0 and ("invalid" in stderr.lower() or "unknown" in stderr.lower() 
                        or "unrecognized" in stderr.lower()):
            self.log(f"Sintassi nuova non supportata, provo legacy...", "INFO")
            cmd_legacy = ["ideviceinstaller"]
            if self.udid:
                cmd_legacy.extend(["-u", self.udid])
            
            if app_type == "all":
                cmd_legacy.extend(["-l", "-o", "list_all"])
            elif app_type == "system":
                cmd_legacy.extend(["-l", "-o", "list_system"])
            else:
                cmd_legacy.extend(["-l", "-o", "list_user"])
            
            stdout, stderr, rc = run_command(cmd_legacy, timeout=30)
        ...
        if rc == 0:
            ...
            for line in stdout.strip().split("\n"):
                ...
                if not line:
                    continue
                # Salta header e righe di totale
                if line.startswith("Total:") or line.startswith("CFBundleIdentifier"):
                    continue
                
                # Formato nuovo: "com.bundle.id, 1.0, BundleName"
                # Formato nuovo variante: "com.bundle.id - AppName 1.0"
                # Formato legacy: "com.bundle.id - AppName 1.0"
                
                if " - " in line:
                    parts = line.split(" - ", 1)
                    bundle_id = parts[0].strip()
                    name_version = parts[1].strip() if len(parts) > 1 else ""
                    # Pulisci bundle_id (potrebbe avere virgola alla fine)
                    bundle_id = bundle_id.rstrip(",").strip()
                    apps.append({"bundle_id": bundle_id, "name_version": name_version})
                elif ", " in line:
                    # Formato CSV: "com.bundle.id, version, DisplayName"
                    parts = [p.strip() for p in line.split(",")]
                    bundle_id = parts[0]
                    version = parts[1] if len(parts) > 1 else ""
                    name = parts[2] if len(parts) > 2 else ""
                    name_version = f"{name} {version}".strip() if name else version
                    apps.append({"bundle_id": bundle_id, "name_version": name_version})
                else:
                    # Riga singola = solo bundle id
    ...
    def cmd_list_apps(self, app_type="user"):
        ...
        def _list():
            ...
            if success:
                ...
                header = f"{'═'*60}\n  Applicazioni {label} - {len(result)} trovate\n{'═'*60}\n\n"
                self.apps_text.insert("end", header)
                
                for i, app in enumerate(result, 1):
                    self.apps_text.insert("end", f"  [{i:3d}] {app['bundle_id']}\n")
                    if app['name_version']:
                        self.apps_text.insert("end", f"        {app['name_version']}\n")
                ...
                if len(result) == 0:
                    self.apps_text.insert("end", "\nNessuna app trovata per questa categoria.\n")
                    if app_type == "user":
                        self.apps_text.insert("end", "Prova con 'Tutte le App' o 'App Sistema'.\n")
            else:
                self.apps_text.insert("end", f"Errore: {result}\n\n")
                self.apps_text.insert("end", "Suggerimenti:\n")
                self.apps_text.insert("end", "  - Verifica che il dispositivo sia sbloccato\n")
                self.apps_text.insert("end", "  - Prova a rifare il pairing\n")
                self.apps_text.insert("end", "  - Controlla la versione di ideviceinstaller:\n")
                self.apps_text.insert("end", "    ideviceinstaller --version\n")
